...
var espelhamentoMatriz = [
    [1,0,0],
    [0,1,0],
    [0,0,1],
]

function Espelhamento(x,y, pol, local){
    espelhamentoMatriz[0][0] = Number(x);
    espelhamentoMatriz[1][1] = Number(y);
    if(local){
        transladar(-pol.Pontos[0].x, -pol.Pontos[0].y,pol)
    }
    for(let i = 0; i < pol.Pontos.length; i++){
        let p = pol.Pontos[i];
        let result = multiplicarMatrizes(espelhamentoMatriz, [[p.x], [p.y], [1]]);
